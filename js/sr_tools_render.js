var render = {
	...
	critter: function($target, data, options)
	{
		...
		switch (options.mode)
		{
			default:
			case 'display': // Default view, includes icons to convert to action and edit modes
				this.critter_for_display($target, data, options);
				break;
			case 'action': // This is similar to display but includes buttons to roll most skills and initiative
				this.critter_for_action($target, data, options);
				break;
			case 'edit': // Lots of dropdowns and widgets to change the NPC
				this.critter_for_edit($target, data, options);
				break;
			case 'print': // Strictly for printing to PDF or other non-interactive modes
				this.critter_for_print($target, data, options);
				break;
		}
	},
	...
	{
		//TODO: for now, just use the existing one, can be changed later
		this.mook_for_display($target, data, options);
	},

	critter_for_print: function($target, data, options)
	{
		//TODO: for now, just use the existing one, can be changed later
		this.critter_for_display($target, data, options);
	},
	...
	critter_for_edit: function($target, data, options)
	{
		var i, $critter = render.get_template('render__edit_critter');
		...
		$target.empty().append($critter);

		// Add buttons to enter other modes
		$critter.find('button').button();

		$critter.find('.controls button.revert').click(function()
		{
			switch (options.view_after_save)
			{
				case 'action':
					render.critter_for_action($target, original_data, options);
					break;

				case 'display':
					render.critter_for_display($target, original_data, options);
					break;

				default:
					console.log('ERROR: critter_for_edit() revert with unknown mode', options.view_after_save);
			}
		});
		...
		var save_mook = function()
		{
			...
			// Name
			if (data.name !== $critter.find('#name').val().trim())
			{
				data.name = $critter.find('#name').val().trim();
				changes_made = true;
			}

			// Notes
			if (data.notes !== $critter.find('#critter_notes').val().trim())
			{
				data.notes = $critter.find('#critter_notes').val().trim();
				changes_made = true;
			}
			...
			{
				data.race = $critter.find('select[name="race"]').val().trim();
				changes_made = true;
			}
			...
			attributes.forEach(function(attribute)
			{
				if (data.attributes[attribute] !== $critter.find('.attribute_values select[name="attribute_' + attribute + '"]').val())
				{
					data.attributes[attribute] = parseInt($critter.find('.attribute_values select[name="attribute_' + attribute + '"]').val());
					changes_made = true;
				}
			});

			// Condition Monitor
			if (data.condition_monitor !== $critter.find('.condition_monitor input[name="condition_monitor"]:checked').attr('id'))
			{
				data.condition_monitor = $critter.find('.condition_monitor input[name="condition_monitor"]:checked').attr('id');
				changes_made = true;
			}

			// Wound Penalties
			if (data.wound_penalty !== $critter.find('.wound_penalty input[name="wound_penalty"]:checked').attr('id'))
			{
				data.wound_penalty = $critter.find('.wound_penalty input[name="wound_penalty"]:checked').attr('id');
				changes_made = true;
			}

			// Armor
			data.armor = $critter.find('.other_information .armor .value select').val();
			...
			switch (options.view_after_save)
			{
				...
				case 'display':
					render.critter_for_display($target, data, options);
					break;

				case 'edit':
					render.critter_for_edit($target, data, options);
					break;
			}
		};

		$critter.find('.controls button.save').click(save_mook);
		...
		var metatype_changed = function()
		{
			...
			attributes.forEach(function(attr)
			{
				...
				$critter.find('.attribute_values .attribute_value.' + attr).empty();
		...
		$critter.find('select[name="race"]').on('change', metatype_changed).change();

		// Condition Monitor
		$critter.find('.other_information .condition_monitor > div').buttonset();
		$critter.find('.other_information .condition_monitor input#' + data.condition_monitor).click();

		// Wound Penalties
		$critter.find('.other_information .wound_penalty > div').buttonset();
		$critter.find('.other_information .wound_penalty input#' + data.wound_penalty).click();

		// Qualities
		var redraw_qualities = function()
		{
			var all_qualities = db.get_quality_list(), $new_quality, $select;
			var $positive_qualities = $critter.find('.other_information .qualities > div.positive');
			var $negative_qualities = $critter.find('.other_information .qualities > div.negative');
		...
		$critter.find('.other_information .qualities > div.quality').on('change', 'select[name="quality_name"]', function()
		{
			var quality = $(this).prop('quality');
			if ($(this).val().length !== 0)
				data.qualities[quality].push($(this).val());
		...
		$critter.find('.other_information .qualities > div.quality').on('click', 'button.remove_quality', function()
		{
			var quality = $(this).prop('quality'), remove = $(this).prop('quality_name'), i = data.qualities[quality].indexOf(remove);
			data.qualities[quality].splice(i, 1);
		...
		var redraw_skills = function()
		{
			var $skill_div = $critter.find('.other_information .skills > div.value'), current_skills, all_skills;
		...
		$critter.find('.other_information .skills > div.value').on('change', 'select[name="skill_name"]', function(e)
		{
			if ($(this).val().length !== 0)
				data.skills[$(this).val()] = 1;
		...
		$critter.find('.other_information .skills > div.value').on('change', 'select[name="skill_rating"]', function(e)
		{
			data.skills[$(this).prop('skill_name')] = parseInt($(this).val());
		...
		$critter.find('.other_information .skills > div.value').on('click', 'button.remove_skill', function(e)
		{
			var remaining_skills = {}, current_skills = Object.keys(data.skills), remove_skill = $(this).prop('skill_name');
		...
		var redraw_augmentations = function()
		{
			var $augmentation_div = $critter.find('.other_information .augments > div.value'), current_augments = [], stock_augment;
		...
		$critter.find('.other_information .augments > div.value').on('change', 'select[name="augmentation_name"]', function(e)
		{
			if ($(this).val().length !== 0)
			{
				var augment_name = $(this).val(), stock_augment = db.get_augmentation(augment_name);
		...
		$critter.find('.other_information .augments > div.value').on('change', 'select[name="augmentation_rating"]', function(e)
		{
			var rating = parseInt($(this).val());
			var name = $(this).prop('augmentation_name');
		...
		$critter.find('.other_information .augments > div.value').on('click', 'button.remove_augmentation', function(e)
		{
			var remaining = [], remove = $(this).prop('augmentation_name');
		...
		// Don't let Mages or Adepts have augmentations
		if (data.special.is_adept === true || data.special.is_mage === true)
			$critter.find('.other_information .augments').hide();
		else
			redraw_augmentations();

		// Armor
		$('<option value=""/>').appendTo($critter.find('.other_information .armor select[name="armor"]'));

		Object.keys(db.get_armor_list()).forEach(function(armor)
		{
			$('<option value="' + armor + '"/>').html(armor).appendTo($critter.find('.other_information .armor select[name="armor"]'));
		});

		if (data.armor)
			$critter.find('.other_information .armor select[name="armor"] option[value="' + data.armor + '"]').prop('selected', true);
		...
		$critter.find('.other_information .weapons > .value').on('change', 'select[name="weapon_name"]', function()
		{
			if ($(this).val().length !== 0)
			{
				var weapon = db.get_weapon($(this).val());
		...
		$critter.find('.other_information .weapons > .value').on('click', 'button.remove_weapon', function()
		{
			var remaining = [], remove = $(this).prop('weapon_name'), removed = false;
	...
	critter_for_action: function($target, data, options)
	{
		var i, $critter = render.get_template('render__action_critter'), wp = {penalty: 0};
		...
		$critter.find('.controls button.display').click(function()
		{
			render.critter_for_display($target, data, options);
		});
		...
		if (data.edited)
		{
			$critter.find('.critter_name').html(data.name + ' -- ' + data.gender + ' ' + data.race);

			$critter.find('.critter_description').hide();
		}
		else
		{
		...
		if (data.notes)
		{
			$critter.find('.critter_notes').html(data.notes);
		}
		else
		{
			$critter.find('.critter_notes').hide();
		}
		...
		if (augmented_attributes.essence % 1 === 0)
		{
			$critter.find('.attribute_values .attribute_value.essence').html(augmented_attributes.essence);
		}
		else if ((augmented_attributes.essence * 10) % 1 === 0)
		{
			$critter.find('.attribute_values .attribute_value.essence').html(augmented_attributes.essence.toFixed(1));
		}
		else
		{
			$critter.find('.attribute_values .attribute_value.essence').html(augmented_attributes.essence.toFixed(2));
		}

		// Set/Hide Magic
		$critter.find('.attribute_values .attribute_value.magic').html(data.special.Magic);
		...
		$critter.find('.information .initiative .value').html(init_display);

		$critter.find('.information .initiative button').button().click(function ()
		{
			var i, total = initiative.base_augmented;
			...
			$critter.find('.information .initiative .result').html(total + wp.penalty);
		});

		if (data.special.is_decker === true)
		{
			var matrix_initiative = this.calc_initiative(data, augmented_attributes, 'matrix');
			$critter.find('.information .matrix_initiative .value').html('Data Processing + ' + matrix_initiative.base + ' + ' + matrix_initiative.dice + 'D6');

			$critter.find('.information .matrix_initiative button').button().click(function ()
			{
				var i, total = matrix_initiative.base;
				...
				$critter.find('.information .matrix_initiative .result').html(total + " + DP");
			});
		}
		else
		{
			$critter.find('.information .matrix_initiative').hide();
		}

		if (data.special.is_mage === true)
		{
			var astral_initiative = this.calc_initiative(data, augmented_attributes, 'astral');
			$critter.find('.information .astral_initiative .value').html(astral_initiative.base + ' + ' + astral_initiative.dice + 'D6');

			$critter.find('.information .astral_initiative button').button().click(function ()
			{
				var i, total = astral_initiative.base;
				...
				$critter.find('.information .astral_initiative .result').html(total + wp.penalty);
			});
		}
		else
		{
			$critter.find('.information .astral_initiative').hide();
		}

		// Condition Monitor
		$critter.find('.information .condition_monitor').append(render.get_condition_monitor(wp, data));
		...
		$critter.find('.information .limits .value').html(limit_display);

		// Qualities
		if (data.qualities.positive.length === 0 && data.qualities.negative.length === 0)
		{
			$critter.find('.information .qualities').hide();
		}
		else
		{
			$critter.find('.information .qualities .value').html(data.qualities.positive.concat(data.qualities.negative).join(', '));
		}

		// Initiate Grade
		if (data.special.hasOwnProperty('Initiate'))
		{
			$critter.find('.information .initiate_grade .value').html(data.special.Initiate);
		}
		else
		{
			$critter.find('.information .initiate_grade').hide();
		}
		...
		for (skill in data.skills)
		{
			...
			$skill = render.get_template('action_skill').appendTo($critter.find('.information .skills .skill_list'));
		...
		if (data.augmentations.length)
		{
			...
			$critter.find('.information .augments .value').html(augments.join(', '));
		}
		else
		{
			$critter.find('.information .augments').hide();
		}
		...
		if (data.armor.length !== 0)
		{
			...
			{
				armor = armors[data.armor];
				$critter.find('.information .armor .value').html(data.armor + ' (' + armor + ')');
				soak += armor;
			}
		}
		else
		{
			$critter.find('.information .armor').hide();
		}

		var dodge_roll = augmented_attributes.reaction + augmented_attributes.intuition;
		$critter.find('.information .dodge .value').html(dodge_roll);
		$critter.find('.information .dodge button').button().click(function ()
		{
			var d = dodge_roll;
			...
			$critter.find('.information .dodge .result').html(total);
		});

		$critter.find('.information .damage_resistance .value').html(soak);
		$critter.find('.information .damage_resistance button').button().click(function ()
		{
			var i = roll.d(soak), total = i.hits;

			$critter.find('.information .damage_resistance .result').html(total);
		});
		...
		for (i in melee)
		{
			...
			$gear = render.get_template('display_weapon').appendTo($critter.find('.information .gear .value'));

			$gear.find('.stats').html(entry.name + ' [' + entry_text.join(', ') + ']');

			$critter.find('.skill_list > div').each(function ()
			{
				if (entry.ability === $(this).prop('skill'))
					$gear.prop('pool', $(this).prop('pool'));
			});
		...
		if (data.special.is_adept === true)
		{
			...
			$critter.find('.information .powers .value').html(powers.join(', '));
		}
		else
		{
			$critter.find('.information .powers').hide();
		}

		// Mage spells
		if (data.special.is_mage === true)
		{
			$critter.find('.information .spells .value').html(data.special.spells.join(', '));
		}
		else
		{
			$critter.find('.information .spells').hide();
		}
		...
		if (data.special.is_decker === true)
		{
			...
			$critter.find('.information .programs .value').html(deck.programs.join(', '));
		}
		else
		{
			$critter.find('.information .programs').hide();
		}
	},
	...
	critter_for_display: function($target, data, options)
	{
		// Given a target, nuke all contents and make a pretty rendering attached to the target
		// will need to calculate augmented attributes, limits, and damage resistance pool
		var i, $critter = render.get_template('render__display_critter');
		...
		{
			initiative = this.calc_initiative(data, augmented_attributes, 'matrix');
			$critter.find('.information .matrix_initiative .value').html('Data Processing + ' + initiative.base + ' + ' + initiative.dice + 'D6');
		}
		else
		{
			$critter.find('.information .matrix_initiative').hide();
		...
		{
			initiative = this.calc_initiative(data, augmented_attributes, 'astral');
			$critter.find('.information .astral_initiative .value').html(initiative.base + ' + ' + initiative.dice + 'D6');
		}
		else
		{
			$critter.find('.information .astral_initiative').hide();
		...
		$critter.find('.information .condition_monitor .value').html(cm);
		...
		if(skills.length > 0)
		{
			$critter.find('.information .skills .value').html(skills.join(', '));
		}
		else
		{
			$critter.find('.information .skills').hide();
		}
